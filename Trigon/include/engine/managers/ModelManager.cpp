...
{
	int bufferLocation			= Renderer::GetInstance()->BindVertexBuffer(mesh->vertexBuffer);
...
ModelManager::LoadModel(const char* path)
{
	...
	for (int i = 0; i < meshCount; i++) 
	{
		...
		for (int vert = 0; vert < geo->getVertexCount(); ++vert)
		{
			...
			tempVert.push_back(Vector3(v.x, v.y, v.z));
			tempNorm.push_back(Vector3(v.x, v.y, v.z));
			tempUV.push_back(Vector2(uv.x, uv.y));
		}

		std::vector<unsigned short> outIndices;
		std::vector<Vector3>		outVert;
		std::vector<Vector3>		outNorm;
		...
		mesh->vertexBufferLocation	=	Renderer::GetInstance()->BindVertexBuffer(mesh->vertexBuffer);
		mesh->uvBufferLocation		=	Renderer::GetInstance()->BindVertexBuffer(mesh->uvBuffer);
		mesh->normalBufferLocation	=	Renderer::GetInstance()->BindVertexBuffer(mesh->normalBuffer);
		mesh->indexBufferLocation	=	Renderer::GetInstance()->BindIndexBuffer(mesh->indexBuffer);
		model->meshes.push_back(mesh);
...
ModelManager::DestroyModelChain(Model* model) 
{
	...
	{
		Renderer::GetInstance()->ReleaseBuffer(model->meshes.at(i)->indexBufferLocation);
		Renderer::GetInstance()->ReleaseBuffer(model->meshes.at(i)->vertexBufferLocation);
		Renderer::GetInstance()->ReleaseBuffer(model->meshes.at(i)->uvBufferLocation);
		Renderer::GetInstance()->ReleaseBuffer(model->meshes.at(i)->normalBufferLocation);
...
{
	Renderer::GetInstance()->ReleaseBuffer(mesh->indexBufferLocation);
	Renderer::GetInstance()->ReleaseBuffer(mesh->vertexBufferLocation);
	Renderer::GetInstance()->ReleaseBuffer(mesh->uvBufferLocation);
	Renderer::GetInstance()->ReleaseBuffer(mesh->normalBufferLocation);
	delete mesh;
}
...
void ModelManager::indexVBO_TBN(
	...
	std::vector<Vector2>& out_uvs,
	std::vector<Vector3>& out_normals,
	std::vector<Vector3>& out_tangents,
	std::vector<Vector3>& out_bitangents
) {
	// For each input vertex
	for (unsigned int i = 0; i < in_vertices.size(); i++) {
