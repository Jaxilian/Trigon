...
bool
modelmanager::LoadObj(const char* path, const char* matPath, Model* model, bool indexed)
{
	...
	if(Scene::currentScene)	Scene::currentScene->models.push_back(model);
...
bool
modelmanager::LoadTex(const char* path, Texture2D* outTexture)
{
	std::string usrPath 		= path;

	outTexture->data = stbi_load((workdirs::assetDir + usrPath).c_str(), &outTexture->width, &outTexture->height, &outTexture->channels, 0);

	if(outTexture->data) return true;
	
	printf("Failed to load texture from %s \n", (workdirs::assetDir + usrPath).c_str());
	throw std::runtime_error("Abort");
	return false;
}

void 
modelmanager::clearTexture(Texture2D* tex)
{
