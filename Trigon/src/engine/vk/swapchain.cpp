...
LveSwapChain::LveSwapChain(VkExtent2D extent)
 : windowExtent{extent} 
 {
  init();
}

LveSwapChain::LveSwapChain(VkExtent2D extent, std::shared_ptr<LveSwapChain> previous)
...
LveSwapChain::~LveSwapChain() {
  ...
  for (int i = 0; i < depthImages.size(); i++) {
    ...
    vkFreeMemory(device::get(), depthImageMemorys[i], nullptr);
  }
...
void LveSwapChain::createSwapChain() {
  SwapChainSupportDetails swapChainSupport = device::getSwapSupp();
  ...
  QueueFamilyIndices indices = device::findPhyQueueFamilies();
...
void LveSwapChain::createImageViews() {
  ...
  for (size_t i = 0; i < swapChainImages.size(); i++) {
    ...
    if (vkCreateImageView(device::get(), &viewInfo, nullptr, &swapChainImageViews[i]) !=
        VK_SUCCESS) {
...
void LveSwapChain::createDepthResources() {
  ...
  for (int i = 0; i < depthImages.size(); i++) {
    ...
    if (vkCreateImageView(device::get(), &viewInfo, nullptr, &depthImageViews[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture image view!");
    }
  }
}
...
void LveSwapChain::createSyncObjects() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateSemaphore(device::get(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) !=
            VK_SUCCESS ||
        vkCreateSemaphore(device::get(), &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) !=
            VK_SUCCESS ||
